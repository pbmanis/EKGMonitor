...
Updated for Python 3 and pyqt6, 3 October 2024.
...
# mode - 'soundcard'  # use system sound card (requires amplifier to go in first)
...
class Arduino:
    """Tell arduino what to do and read the data back."""
    ...
    def wait_response(self, timeout: float = 2):
        # set time to wait for a response, in seconds
        a = time.time()
        while self.serbuf.inWaiting() == 0:
            if (time.time() - a) < timeout:
                pass
            else:
                print("Arduino timed out on wait")
                return
    ...
    def print_ard_info(self):
        ...
        print("Arduino information response: ", c)

    def write_data(self, data):
        self.serbuf.write(data.encode("utf-8"))
    ...
    def set_sample(self, rate, points):
        ...
        self.write_data(f"s {int(rate*1e6):d};\n")
        print("Rate in microseconds: ", int(rate * 1e6))  # microseconds
        self.write_data(f"n {int(points):d};\n")
        print("# of points: ", int(points))
        #    self.wait_response()
        print(self.read_response())
        #    ard_flushbuf(s)
        self.sampleduration = points * rate
        print("Sampling epoch in seconds: ", self.sampleduration)
        self.write_data("i")
        print("Interrogate Arduino: ")
        #    self.wait_response()
        print(self.read_response())
...
if mode == "olimex":
    ...
    source = "/dev/cu.usbmodem24201"  # 'COM5' # /dev/cu.usbmodem621'
...
def checkfs():
    ...
    for fs in possibleSamplerates:
        try:
            sd.check_input_settings(device=device, samplerate=int(1.0 / fs), channels=NChannels)
        except Exception as e:
            print("Sample rate is not recognized: ", fs, e)
        else:
...
class MeasureECG:
    ...
    def __init__(self, finfo=None, input_mode="soundcard"):
        self.analysisSampleFreq = (
            1000.0  # Hz, for the data we will use (not the rate at which the signal is sampled)
        )
        self.sampleFreq = self.analysisSampleFreq  # actual frequency after decimation
        if input_mode == "soundcard":
    ...
    def setfs(self, fs):
        ...
        self.decimate = int((1.0 / self.fs) / self.analysisSampleFreq)  # decimate to about 1 kHz
    ...
    def LPFilter(self, data, fc=40.0, numtaps=5):
        """
    ...
    def NotchFilter(self, data, fn=60.0, Q=50.0):
        """
        ...
        b, a = scipy.signal.iirdesign(wp, ws, gpass=1.0, gstop=60.0)
        dfilt = scipy.signal.lfilter(b, a, data)
        return dfilt

    def loadFile(self, fname: str, startAt: int = 0, length: int = None, Hz: float = 1000):
        """
        ...
        if finfo["fs"] is not None:
            ...
            self.setfs(Hz)
        self.xs = np.linspace(
            0.0, len(self.currentSegment) * Hz, len(self.currentSegment)
        )  # generate x time base axis
        ...
        if (
            len(self.currentSegment.shape) > 1
        ):  # depends on how many channels recorded and which needed
            self.currentSegment = scipy.signal.decimate(
                self.currentSegment[:, finfo["channel"]], self.decimate
            )
        else:
    ...
    def captureSegment(self, duration: float = 1.0):
        """
        ...
        if self.device is not None:
            try:
                sd.check_input_settings(
                    self.device, samplerate=int(self.fs), channels=self.NChannels
                )
            except:
                raise ValueError("Invalid sample rate for input device")
            self.currentSegment = sd.rec(
                int(duration / self.fs), samplerate=int(1.0 / self.fs), blocking=True, channels=2
            )
            self.currentSegment = scipy.signal.decimate(self.currentSegment[:, 1], self.decimate)
            self.sampleFreq = (1.0 / self.fs) / self.decimate
            self.lastTimes = np.linspace(0, duration, self.currentSegment.shape[0])
        else:  # read from adruino/olimex
            Ard.send_command("a")
            time.sleep(Ard.sampleduration * 2.0 + 0.2)
            Ard.wait_response(timeout=2.0)
            ...
            self.sampleFreq = 1.0 / self.fs  # /self.decimate
            # print("Sample Frequency: ", self.sampleFreq)
...
class Updater:
    ...
    def setSampling(self, interval: float = 5.0, duration: float = 1.0):
        """
    ...
    def setMaxSamples(self, maxsamples:int=3):
        """
        Parameters
        ----------
    ...
    def makeFilename(self):
        ...
        self.filename = "{0:04d}.{1:02d}.{2:02d}_{3:02d}.{4:02d}.{5:02d}.p".format(
            n.year, n.month, n.day, n.hour, n.minute, n.second
        )
        acqProp = self.ptreedata.child("Acquisition Parameters")
        acqProp["Filename"] = self.filename

    def change(self, param:list, changes: list):
        """
    ...
    def continueRun(self):
        ...
        self.timer.start(int(self.readInterval * 1000))
        self.timedWrite.start(int(10 * 1000.0))  # update file in 10 second minute increments
    ...
    def storeData(self):
        ...
        data = {
            ...
            "runningRate": self.runningRate,
            "runningVar": self.runningVar,
            "RRInterval": self.RRInterval,
            "out": [x.as_dict() for x in self.out],
            "LPFFreq": self.LPFFreq,
            "NotchFreq": self.NotchFreq,
            "NotchEnabled": self.NotchEnabled,
    ...
    def update(self):
        ...
        filtered_signal = self.ecg.LPFilter(
            self.ecg.currentSegment, fc=self.LPFFreq / self.ecg.sampleFreq
        )
        if self.ecg.NotchEnabled:
            filtered_signal = self.ecg.NotchFilter(
                filtered_signal, fn=self.NotchFreq / self.ecg.sampleFreq
            )
        ctime = datetime.datetime.now()
        self.runtime = (ctime - self.startTime).seconds / 60.0
        self.pltd["plt_first"].plot(
            self.ecg.lastTimes, filtered_signal, clear=True, pen=pg.mkPen("g")
        )
        try:  # do analysis on potential ecg signal
            self.out.append(
                b_ecg.ecg(
                    signal=filtered_signal,
                    sampling_rate=self.ecg.sampleFreq,
                    show=False,
                    before=0.1,
                    after=0.15,
                )
            ...
            print(f"No beats detected at {stime:s}")
            self.NSamples = self.NSamples + 1
            # then plot to the template window to show us what is really there
            self.pltd["plt_first"].plot(
                self.ecg.lastTimes, self.ecg.currentSegment, clear=True, pen=pg.mkPen("r")
            )
            return
    ...
    def plotResults(self, readmode=False):
        ...
        if self.NSamples == 0 or readmode is True:
            for j in range(self.out[-1]["templates"].shape[0]):
                self.pltd["plt_first"].plot(
                    self.out[-1]["templates_ts"],
                    self.out[-1]["templates"][j],
...
if __name__ == "__main__":
    ...
    updater = Updater(
        ...
        pltd={
            ...
            "plt_var": plt_var,
            "plt_hr": plt_hr,
            "plt_current": plt_current,
            "plt_RRI": plt_RRI,
        },
        ptree=ptreedata,
        invert=ecg.invertData,
        notchEnabled=ecg.NotchEnabled,
    )
